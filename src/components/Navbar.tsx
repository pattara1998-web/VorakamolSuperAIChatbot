...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className={`${theme === 'dark' ? 'bg-[#0C0C0E] text-zinc-100 border-zinc-800/80' : 'bg-white text-zinc-900 border-slate-200 shadow-sm'} border-b sticky top-0 z-40 shrink-0`}>
      {/* Top Banner Bar */}
      <div className={`${theme === 'dark' ? 'bg-[#121216] border-zinc-800/60 text-zinc-300' : 'bg-slate-100 border-slate-200 text-zinc-700'} border-b px-4 py-1.5 text-xs flex items-center justify-between font-mono shrink-0`}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            ENGINE STATUS: <span className="text-emerald-500 font-semibold">100% STANDALONE WEB APP</span> • 
            {isApiConfigured ? (
              <span className="text-emerald-500 font-semibold"> AI พร้อมใช้งาน</span>
            ) : (
              <span className="text-amber-500 font-semibold flex items-center gap-1">
                <AlertCircle className="w-3 h-3" /> ต้องใส่ API AI ก่อนใช้งาน
              </span>
            )}
          </span>
        </div>
        <div className="hidden md:flex items-center gap-4">
          ...
          >
            ...
            {isApiConfigured ? 'AI พร้อม' : 'ใส่ API AI'}
          </button>
          ...
          <button
            ...
            className="hover:text-indigo-400 flex items-center gap-1 font-semibold transition-colors text-zinc-600 dark:text-zinc-300 cursor-pointer"
          >
            ซิงค์ Facebook 1-Click <ExternalLink className="w-3 h-3" />
          </button>
        </div>
      </div>

      {/* Main Nav Header */}
      <div className="w-full max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-8 shrink-0">
